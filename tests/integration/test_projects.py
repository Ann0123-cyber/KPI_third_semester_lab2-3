class TestCreateProject:
    ...
    def test_no_auth_returns_403(self, client):
        assert client.post("/projects/", json={"name": "Alpha"}).status_code == 403

    def test_duplicate_name_returns_409(self, client, auth_headers):
        client.post("/projects/", json={"name": "Alpha"}, headers=auth_headers)
        assert client.post("/projects/", json={"name": "Alpha"}, headers=auth_headers).status_code == 409

    def test_empty_name_returns_422(self, client, auth_headers):
        assert client.post("/projects/", json={"name": "  "}, headers=auth_headers).status_code == 422

class TestListProjects:
    def test_returns_only_own(self, client, auth_headers):
    ...
    def test_other_users_projects_not_visible(self, client, auth_headers):
        client.post("/projects/", json={"name": "P1"}, headers=auth_headers)
        client.post("/auth/register", json={"email": "bob@e.com", "username": "bob", "password": "secret123"})
        bob_token = client.post("/auth/login", json={"email": "bob@e.com", "password": "secret123"}).json()["access_token"]
        resp = client.get("/projects/", headers={"Authorization": f"Bearer {bob_token}"})
        assert resp.json() == []

class TestGetProject:
    def test_get_existing(self, client, auth_headers, project):
    ...
    def test_get_nonexistent_returns_404(self, client, auth_headers):
        assert client.get("/projects/9999", headers=auth_headers).status_code == 404

class TestUpdateProject:
    def test_rename_project(self, client, auth_headers, project):
        resp = client.patch(f"/projects/{project['id']}", json={"name": "Renamed"}, headers=auth_headers)
    ...
    def test_rename_to_existing_name_returns_409(self, client, auth_headers):
        ...
        client.post("/projects/", json={"name": "P2"}, headers=auth_headers)
        assert client.patch(f"/projects/{p1['id']}", json={"name": "P2"}, headers=auth_headers).status_code == 409
...
class TestDeleteProject:
    ...
    def test_delete_nonexistent_returns_404(self, client, auth_headers):
        assert client.delete("/projects/9999", headers=auth_headers).status_code == 404
